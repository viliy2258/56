    ...
    if (telegramUserId) {
        ...
        set(userRef, {
            ...
            boosts: {
                energyLimit: {
                    lvl: upgradeLevel,
                    cost: baseCost + (upgradeLevel * 500) + (upgradeLevel * 500)
                },
                energyRechargeSpeed: {
                    lvl: rechargeLevel,
                    cost: 1000 + (rechargeLevel * 1000) + (rechargeLevel * 500)
                },
                multitap: {
                    lvl: tapLevel,
                    cost: baseCost + (tapLevel * 500) + (tapLevel * 500)
                }
            }
        });
    }
            ...
            if (snapshot.exists()) {
                ...
                if (data.boosts) {
                    if (data.boosts.energyLimit) {
                        ...
                        baseCost = data.boosts.energyLimit.cost - ((upgradeLevel - 1) * 500) - ((upgradeLevel - 1) * 500); // Calculate baseCost if needed
                    }
...
function processPurchase(item) {
    ...
    if (item.dataset.boost === 'energy-limit') {
        cost = baseCost + (level - 1) * 500 + ((level - 1) * 500);
    } else if (item.dataset.boost === 'energy-recharge-speed') {
        cost = 1000 + (level - 1) * 1000 + ((level - 1) * 500);
    ...
    if (balance >= cost) {
        ...
        // Save the updated cost to Firebase
        saveDataToFirebase();
...
function updateBoostCost() {
    const energyLimitCost = baseCost + (upgradeLevel * 500) + (upgradeLevel * 500);  // Increase by 500 for each level
    document.querySelector('.boost-item[data-boost="energy-limit"] .boost-cost').innerText = energyLimitCost.toLocaleString();

    const rechargeSpeedCost = 1000 + (rechargeLevel * 1000) + (rechargeLevel * 500);  // Increase by 500 for each level
    document.querySelector('.boost-item[data-boost="energy-recharge-speed"] .boost-cost').innerText = rechargeSpeedCost.toLocaleString();

    const tapMultiplierCost = baseCost + (tapLevel * 500) + (tapLevel * 500);  // Increase by 500 for each level
